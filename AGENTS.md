...
- 테스트: 133 top-level tests (Go std test + race/TZ gates + benches)
...
**최신 데이터: 2026-07-13** (Samsung export 2026-07-14 덤프, `lifetract.db` 202,479 rows).
...
>
> 아래 여섯 조항은 **테스트로 강제된다** (`lifetract/timeaxis_test.go`,
...
경계는 **반드시 KST 자정**이다. `--days N` 은 `[내일-N, 내일)`, 즉 오늘을 포함한
**정확히 N개 달력일**이다 (`--days 1` = 오늘). 단독이든 `--to`와 조합하든 폭의 뜻은 같다.
"지금부터 N 일 전" 같은 한낮의 instant 를 경계로 쓰면 창의 가장 오래된 날이 잘려서
돌아온다 — `--days 3` 과 `--days 5` 가 같은 과거 날짜에 대해 다른 숫자를 낸다.
**창은 렌즈지 편집이 아니다. 창을 넓힌다고 과거가 바뀌면 안 된다.**
...
**5. 잃음도 스스로 신고하고, 잃은 DB 는 운영에 승격되지 않는다.**

`import --exec` 는 스트림별 행수를 **직전 import 와 대조**하고, 잃었으면
`status: "warning"` + `warnings[]` 를 낸다 (`import_ledger.go`).

**말하는 것만으로는 부족하다.** 예전엔 운영 DB 를 먼저 지우고 그 자리에 새로 지었다 —
경고가 찍힐 때쯤엔 스트림을 아직 갖고 있던 DB 가 이미 없었고, 이후 모든 조회가 손상된
쪽을 읽었다. 이제 **후보(`lifetract.db.candidate`)에 짓고 성한 run 만 원자적으로 승격**
한다 (`promoteDB`). 잃은 run 은 `candidate_path` 로 남겨 조사하게 두고, 운영 DB 는
건드리지 않는다. **잃었다고 말하면서 잃은 DB 를 넘겨주면 그 경고는 묘비명이다.**

승격을 막는 것은 셋이다 — 스트림 손실 / 읽지 못한 소스 / **원장 기록 실패**. 원장을
못 쓴 DB 를 승격하면 다음 import 가 자기를 첫 import 로 착각한다. 그래서 원장의 모든
오류(`carryForward` · `logImport` · `rows.Err()` · 스캔 실패)는 삼키지 않고 판정에
반영한다 — *검사기가 조용히 검사를 그만두는 것도 침묵이다.*
...
- **첫 import 는 *손실* 을 주장하지 않는다** (비교 대상 없음). `note` 가 그렇게 말한다.
  그러나 **못 읽은 소스는 첫 import 라도 `ok` 가 아니다** — "비교할 게 없다" 는 손실을
  주장하지 않을 이유지, 못 읽은 소스를 성하다고 부를 이유가 아니었다.
