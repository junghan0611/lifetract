...
| Home Assistant REST | live state + recorder (30일 보관) | 2026-05-17 ~ | 폰 Companion App → HA → on-demand fetch. SSOT 아닌 라이브 인터페이스. |

Samsung CSV 가 본 SSOT (주기 덤프 → 9년치 timeline 보존), HA 는 *오늘 자리* 라이브 인터페이스. 두 source 가 5/17 자리에서 겹쳐 시간축 단절 없음. `today` / `read <오늘>` 호출 시 DB 가 빈 자리는 자동으로 HA 가 채움 (phase 7 read-only fallback, [docs/plan.md](docs/plan.md) §Phase 7).
